class OffersController < ApplicationController
  ...
  def topup   
     ...
     redirect_to URI.encode("https://www.sandbox.paypal.com/cgi-bin/webscr" + "?cmd=_xclick&business=#{APP_CONFIG[:paypal_email]}&currency_code=USD&item_name=#{params[:offer_name]}&amount=#{params[:amount]}&invoice=#{params[:offer_id]}&rm=#{params[:rm]}&return=#{params[:return_url]}&quantity=1&notify_url=#{notify_url}")     
     return
  end
